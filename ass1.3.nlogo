...
to setup
  ...
  reset-timer
  set time timer
  set-default-shape vacuums "circle 2"
  set-default-shape containers "truck"
  setup-patches
  setup-vacuum
  setup-container
  setup-ticks
...
to setup-patches
  ...
  ; calculate amount of dirty patches
  let dirty-tiles count patches * dirt_pct / 100
  set total_dirty floor dirty-tiles

  ; colour in all patches
  ask patches [ set pcolor white ]
  ask n-of dirty-tiles patches [ set pcolor brown ]
end
...
to update-desires
  ...
  ifelse any? patches with [ pcolor = brown ]
    [ask vacuums [set desire "clean"]]
    [ask vacuums [set desire "turn off"]]
end
  ...
  if [collected_dirt] of vacuum 0 < bag_size and [intention_act] of vacuum 0 = "clean"
      ...
      [ask vacuums [
        if pcolor = brown [
          ...
          set collected_dirt collected_dirt + 1
          set intention_act "go"
          set intention_loc 0
        ]]]
end

; --- Empty collection of dirt ---
to empty

  ; if vacuum is full and located at the container, empty collection
  if [intention_act] of vacuum 0 = "empty"
     [ask vacuum 0
       [ set collected_dirt 0
       set intention_act "go"
       set intention_loc 0]]
